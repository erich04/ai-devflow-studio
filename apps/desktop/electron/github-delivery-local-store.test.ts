...
describe('GitHub Delivery Intent local persistence', () => {
  ...
  it('keeps schema 17 revocation checks isolated after migrating through schema 32', async () => {
    const dbPath = await tempDbPath()
    const store = await createLocalStore({ dbPath })
  ...
  it('preserves an existing v14 JSON series and non-first attempt through schema 32', async () => {
    const dbPath = await tempDbPath()
    const sources = createSources()
    const store = await createLocalStore({ dbPath })
    await saveSources(store, sources)
