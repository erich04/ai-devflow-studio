...
const dropAgentCoordinationSchemaSql = `
  ...
  drop table if exists agent_coordination_handoffs;
  drop table if exists agent_coordination_tasks;
  drop table if exists agent_coordination_graphs;
  drop table if exists agent_coordination_sessions;
`
...
async function persistAcceptedMemoryCandidate(
  ...
  const created = createAgentRuntime(startRequest)
...
describe('createLocalStore', () => {
  it('migrates retained schema 29 Coding Diff artifacts through the versioned privacy sweep', async () => {
    ...
    retained.run('drop table if exists github_delivery_content_scans')
    retained.run(
      `insert into coding_diff_artifacts (id, run_id, node_id, project_id, json, created_at)
       values (?, ?, ?, ?, ?, ?)`,
      [
    ...
    retained.run("drop table if exists workbench_conversations; update schema_meta set value = '29' where key = 'schema_version'")
  ...
  it('keeps schema 29 durable when the post-migration Coding Diff sweep fails', async () => {
    ...
    retained.run(`
      ...
      drop table coding_diff_artifacts_current;
      drop table if exists workbench_conversations; update schema_meta set value = '29' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
  ...
  it('persists privacy provenance and indexes only rows that still need maintenance', async () => {
    const dbPath = await tempDbPath()
    ...
    for (const [table, orderColumn] of [
      ...
      ['coding_agent_events', 'timestamp'],
      ['test_evidence', 'created_at'],
    ] as const) {
      expect(
        inspected.exec(`select distinct privacy_version from ${table}`)[0]?.values,
      ).toEqual([[1]])
      expect(
        inspected.exec(
          `explain query plan select json from ${table}
           where privacy_version is null or privacy_version < 1
           order by ${orderColumn} asc, id asc`,
        )[0]?.values.map((row) => String(row[3])),
      ).toEqual([expect.stringContaining(`idx_${table}_pending_privacy`)])
    }
    inspected.close()
  })
  ...
  it('migrates retained schema 21 to 22 without fabricating retrieval index rows', async () => {
    ...
    const retained = new SQL.Database(await readFile(dbPath))
    retained.run(`
      ${dropAgentCoordinationSchemaSql}
      ${dropAgentMemorySchemaSql}
      ${dropKnowledgeIndexSchemaSql}
      drop table if exists workbench_conversations; update schema_meta set value = '21' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
  ...
  it('migrates retained schema 22 to 23 without fabricating Memory candidates', async () => {
    ...
    retained.run(`
      ${dropAgentCoordinationSchemaSql}
      ${dropAgentMemorySchemaSql}
      drop table if exists workbench_conversations; update schema_meta set value = '22' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
  ...
  it('migrates retained schema 23 to 24 without fabricating durable Memory lifecycle rows', async () => {
    ...
    retained.run(`
      ...
      drop table if exists agent_memory_heads;
      drop table if exists agent_memory_revisions;
      drop table if exists workbench_conversations; update schema_meta set value = '23' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
  ...
  it('migrates retained schema 25 to an empty bounded Runtime Context store', async () => {
    ...
    const retained = new SQL.Database(await readFile(dbPath))
    retained.run(dropAgentCoordinationSchemaSql)
    retained.run('drop table agent_runtime_context_attachments')
    retained.run("drop table if exists workbench_conversations; update schema_meta set value = '25' where key = 'schema_version'")
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
  ...
  it('atomically enqueues one metadata-only Team Memory projection on promotion', async () => {
    const dbPath = await tempDbPath()
    const store = await createLocalStore({ dbPath })
    await store.saveDesktopPairingCredential(
      { ...desktopPairingCredential, localProjectId: project.id },
    ...
    const { candidate } = await persistAcceptedMemoryCandidate(
      store,
      teamAgentRuntimeStartRequest,
    )
    const authorization = await store.authorizeAgentMemoryPromotion({
      candidateId: candidate.id,
      memoryId: 'memory-team-sync-1',
      authority: {
        stateVersion: 1,
        decisionId: 'memory-team-sync-promotion-1',
        candidateId: candidate.id,
        candidateContentDigest: candidate.contentDigest,
        scope: candidate.scope,
        actorKind: 'human',
        actorId: candidate.scope.userId,
        policyId: 'memory-policy-team-sync-1',
        policyVersion: 1,
        visibility: 'user_project',
        sensitivity: 'private',
        retentionClass: 'until_deleted',
    ...
    expect(authorization.authorized).toBe(true)
    if (!authorization.authorized) throw new Error('expected Team Memory promotion authority')
    ...
    const operation = (await store.listRemoteSyncOperations()).find(
      (candidate) => candidate.kind === 'agent-memory-summary',
    )
    expect(operation).toMatchObject({
      ...
      runId: gateWorkflowCreation.run.id,
      entityId: authorization.revision.id,
      status: 'pending',
    })
    expect(JSON.stringify(operation)).not.toMatch(
      /statement|contentDigest|localProjectPath|sessionId|prompt|reasoning|credential|rawOutput/iu,
    )
    await expect(
      store.getAgentMemoryTeamProjectionInput(authorization.revision.id),
    ).resolves.toMatchObject({
      memory: {
        memoryId: authorization.revision.id,
        statement: authorization.revision.statement,
        headVersion: 1,
        lifecycleStatus: 'active',
      },
      runId: gateWorkflowCreation.run.id,
      nodeId: gateWorkflowCreation.run.currentNodeId,
      runtimeId: teamAgentRuntimeStartRequest.id,
      citationIds: [],
      retrievalCount: 0,
      acceptedContextCount: 0,
      qualityVersion: 1,
      qualityUpdatedAt: authorization.revision.createdAt,
    })

    const promotedHead = await store.getAgentMemoryHead(authorization.revision.id)
    if (promotedHead === null) throw new Error('expected promoted Team Memory head')
    const qualityAttachedAt = '2026-08-12T20:30:05.500Z'
    const qualityAttachment = await assembleAgentRuntimeContext({
      id: 'runtime-context-team-memory-quality-1',
      runtimeId: 'agent-runtime-team-memory-consumer-1',
      checkpointVersion: 1,
      scope: authorization.revision.scope,
      authority: teamAgentRuntimeStartRequest.authority,
      citationSources: [],
      memorySources: [{
        revision: authorization.revision,
        current: {
          ...
          headVersion: promotedHead.version,
          status: 'active',
          scope: authorization.revision.scope,
          sourceRuntimeId: teamAgentRuntimeStartRequest.id,
          contentDigest: authorization.revision.contentDigest,
          updatedAt: promotedHead.updatedAt,
        },
      }],
      attachedAt: qualityAttachedAt,
    })
    const qualityRuntime = createAgentRuntime({
      ...teamAgentRuntimeStartRequest,
      id: qualityAttachment.runtimeId,
      contextDigest: qualityAttachment.contextDigest,
      requestedAt: qualityAttachedAt,
      deadline: '2026-08-12T20:31:05.500Z',
    })
    await expect(store.commitAgentRuntimeTransition({
      expectedRuntime: null,
      transition: qualityRuntime,
      contextAttachment: qualityAttachment,
    })).resolves.toMatchObject({ committed: true, replayed: false })
    await expect(
      store.getAgentMemoryTeamProjectionInput(authorization.revision.id),
    ).resolves.toMatchObject({
      memory: { headVersion: 1, lifecycleStatus: 'active' },
      qualityVersion: 2,
      qualityUpdatedAt: qualityAttachedAt,
      retrievalCount: 1,
      acceptedContextCount: 1,
    })

    const revisionAuthorization = await store.authorizeAgentMemoryRevision({
      memoryId: authorization.revision.id,
      expectedHeadVersion: 1,
      statement: 'The Team Memory projection advances without exporting this replacement statement.',
      authority: {
        stateVersion: 1,
        decisionId: 'memory-team-sync-revision-2',
        memoryId: authorization.revision.id,
        expectedRevision: 1,
        expectedContentDigest: authorization.revision.contentDigest,
        scope: authorization.revision.scope,
        actorKind: 'human',
        actorId: authorization.revision.scope.userId,
        policyId: 'memory-policy-team-sync-1',
        policyVersion: 2,
        visibility: 'user_project',
        sensitivity: 'private',
    ...
    expect(revisionAuthorization.authorized).toBe(true)
    if (!revisionAuthorization.authorized) throw new Error('expected Team Memory revision authority')
    await expect(store.commitAgentMemoryRevision({
      revision: revisionAuthorization.revision,
      recordedAt: '2026-08-12T20:30:06.000Z',
    }, revisionAuthorization.capability)).resolves.toMatchObject({
      committed: true,
      replayed: false,
    })
    await expect(store.listRemoteSyncOperations()).resolves.toEqual(expect.arrayContaining([
    ...
    const deletionAuthorization = await store.authorizeAgentMemoryDeletion({
      authority: {
        stateVersion: 1,
        decisionId: 'memory-team-sync-deletion-3',
        memoryId: authorization.revision.id,
        expectedRevision: 2,
        expectedHeadVersion: 2,
        expectedContentDigest: revisionAuthorization.revision.contentDigest,
        scope: revisionAuthorization.revision.scope,
        actorKind: 'human',
        actorId: revisionAuthorization.revision.scope.userId,
        policyId: 'memory-policy-team-sync-1',
        policyVersion: 3,
    ...
    expect(deletionAuthorization.authorized).toBe(true)
    if (!deletionAuthorization.authorized) throw new Error('expected Team Memory deletion authority')
    await expect(store.commitAgentMemoryDeletion(
      { tombstone: deletionAuthorization.tombstone },
      deletionAuthorization.capability,
    )).resolves.toMatchObject({ committed: true, replayed: false })
    await expect(
      store.getAgentMemoryTeamProjectionInput(authorization.revision.id),
    ).resolves.toMatchObject({
      memory: { headVersion: 3, lifecycleStatus: 'purge_pending', statement: expect.any(String) },
    })

    await expect(store.purgeAgentMemoryDerivedState({
      memoryId: authorization.revision.id,
      expectedDeletionVersion: deletionAuthorization.tombstone.deletionVersion,
      purgedAt: '2026-08-12T20:30:08.000Z',
    })).resolves.toMatchObject({ purged: true, replayed: false })
    await expect(
      store.getAgentMemoryTeamProjectionInput(authorization.revision.id),
    ).resolves.toMatchObject({
      memory: { headVersion: 4, lifecycleStatus: 'deleted', statement: null },
    })
    const finalOperation = (await store.listRemoteSyncOperations()).find(
      (candidate) => candidate.kind === 'agent-memory-summary',
    )
    expect(finalOperation).toMatchObject({ generation: 5, status: 'pending' })
    expect(JSON.stringify(finalOperation)).not.toContain(revisionAuthorization.revision.statement)
    store.close()
  })
  ...
  it('migrates Desktop schema 19 without promoting Team MCP metadata into local authority', async () => {
    ...
    legacy.run(dropAgentCoordinationSchemaSql)
    legacy.run(`
    ...
    legacy.run("drop table if exists workbench_conversations; update schema_meta set value = '19' where key = 'schema_version'")
  ...
  it('migrates Desktop schema 17 without changing 1.x state or inventing runtime rows', async () => {
    ...
    legacy.run(dropAgentMemorySchemaSql)
    legacy.run(dropKnowledgeIndexSchemaSql)
    legacy.run("drop table if exists workbench_conversations; update schema_meta set value = '17' where key = 'schema_version'")
    await writeFile(dbPath, Buffer.from(legacy.export()))
    legacy.close()
  ...
  it('migrates Desktop schema 18 by binding durable grants to exact permission and resource scope', async () => {
    ...
    legacy.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '18' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(legacy.export()))
    legacy.close()
  ...
  it('atomically creates, replays, and reopens one Supervisor-owned coordination session', async () => {
    ...
    await expect(store.listRemoteSyncOperations(gateWorkflowCreation.run.id)).resolves.toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: 'agent-coordination-summary',
          runId: gateWorkflowCreation.run.id,
          entityId: coordinationSessionRequest.id,
          status: 'pending',
        }),
      ]),
    )
    await expect(
      store.getAgentCoordinationTeamProjectionInput(coordinationSessionRequest.id),
    ).resolves.toMatchObject({
      session: {
        coordinationId: coordinationSessionRequest.id,
        localProjectId: project.id,
  ...
  it('migrates an existing v1 database to v28 without losing local projects or runs', async () => {
    const dbPath = await tempDbPath()
  ...
  it('migrates retained schema 27 through schema 36 without fabricating coordination state', async () => {
    ...
    retained.run(`
      ...
      drop table if exists agent_coordination_graphs;
      drop table if exists agent_coordination_sessions;
      drop table if exists workbench_conversations; update schema_meta set value = '27' where key = 'schema_version';
    `)
    await writeFile(dbPath, Buffer.from(retained.export()))
    retained.close()
    ...
    for (const tableName of [
      ...
      'agent_coordination_graphs',
      'agent_coordination_tasks',
      'agent_coordination_handoffs',
      'agent_coordination_leases',
      'agent_coordination_audits',
      ...
      expect(inspected.exec(`select count(*) from ${tableName}`)[0]?.values[0]?.[0]).toBe(0)
    }
    inspected.close()
  })
  ...
  it('migrates v8 data into a metadata-only remote-sync outbox schema', async () => {
    ...
    v8Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '8' where key = 'schema_version';
    `)
  ...
  it('migrates a retained v20 outbox through schema 36 without losing queued metadata', async () => {
    ...
    v20Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '20' where key = 'schema_version';
    `)
  ...
  it('migrates a retained schema 26 outbox to metadata-only Agent Memory sync', async () => {
    const dbPath = await tempDbPath()
    const retainedOperation = createRemoteSyncOperation({
      id: 'sync-retained-v26',
      kind: 'agent-runtime-summary',
      localProjectId: project.id,
      runId: run.id,
      entityId: 'agent-runtime-retained-v26',
    ...
    const v26Db = new SQL.Database(await readFile(dbPath))
    v26Db.run(dropAgentCoordinationSchemaSql)
    v26Db.run("drop table if exists workbench_conversations; update schema_meta set value = '26' where key = 'schema_version'")
    await writeFile(dbPath, Buffer.from(v26Db.export()))
    v26Db.close()
    ...
    await expect(migrated.enqueueRemoteSyncOperation(createRemoteSyncOperation({
      ...
      localProjectId: project.id,
      runId: run.id,
      entityId: 'agent-memory-team-1',
      createdAt: '2026-08-13T12:01:00.000Z',
    }))).resolves.toMatchObject({ operation: { kind: 'agent-memory-summary' } })
    migrated.close()
  })
  ...
  it('migrates a retained v9 database to the Work Request materialization schema', async () => {
    ...
    v9Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '9' where key = 'schema_version';
    `)
  ...
  it('migrates a retained v10 database to the Gate Command execution schema', async () => {
    ...
    v10Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '10' where key = 'schema_version';
    `)
  ...
  it('migrates retained v11 Gate receipts into evaluated metadata observations', async () => {
    ...
    v11Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '11' where key = 'schema_version';
    `)
  ...
  it('leaves the previous version and data intact when a migration transaction fails', async () => {
    ...
    v7Db.run(`
      ...
      drop table if exists workbench_conversations; update schema_meta set value = '7' where key = 'schema_version';
    `)
  ...
  it('permanently redacts legacy Test Evidence when opening an existing database', async () => {
    ...
    legacyDb.run('update test_evidence set json = ?, privacy_version = null where id = ?', [
  ...
  it('permanently rebuilds a legacy Test Result event from sanitized evidence', async () => {
    ...
    legacyDb.run('update agent_events set json = ?, privacy_version = null where id = ?', [
  ...
  it('permanently rebuilds legacy Test Evidence reports without changing non-TestEvidence artifacts', async () => {
    ...
    legacyDb.run('update artifacts set json = ?, privacy_version = null where id = ?', [
  ...
  it('persists local agent reviews, traces, and token usage across reopen', async () => {
    const dbPath = await tempDbPath()
    const typedAgentReview: AgentReviewResult = {
      ...agentReview,
      knowledgeReferences: [{
        id: 'knowledge-reference-review-1',
        runId: agentReview.runId,
        targetType: 'node',
        nodeId: agentReview.nodeId,
        documentId: 'testing-standard-1',
        chunkId: 'testing-standard-chunk-1',
        relation: 'cites',
        reason: 'A lexical retrieval candidate reviewed by the Gate agent.',
        category: 'testing_standard',
        strategy: 'lexical',
        lexicalMatch: {
          rawScore: 6,
          matchedTerms: ['test', 'evidence'],
          normalized: false,
    ...
    const first = await createLocalStore({ dbPath })
    await first.saveAgentReview(typedAgentReview)
    await first.saveAgentTrace(agentTrace)
    await first.saveAgentTokenUsage(agentTokenUsage)
    first.close()

    const second = await createLocalStore({ dbPath })
    expect(await second.listAgentReviews('run-1')).toEqual([typedAgentReview])
  ...
  it('migrates and persists project Coding configuration plus immutable v2 Change Sets', async () => {
    const dbPath = await tempDbPath()
    const configuration = {
      projectId: project.id,
      executor: 'native-model' as const,
      providerId: 'deepseek',
      version: 1,
      updatedAt: '2026-08-29T09:00:00.000Z',
    }
    const { completedAt: _completedAt, ...activeCodingRun } = codingRun
    const nativeRun: CodingAgentRun = {
      ...activeCodingRun,
      providerId: configuration.providerId,
      engine: 'native',
      configVersion: configuration.version,
      changeSetId: 'coding-change-set-1',
      status: 'waiting_permission',
      changedPaths: [],
      summary: 'Waiting for exact Change Set approval.',
    }
    const nativeWorkspace: ManagedCodingWorkspace = {
      ...workspace,
      codingRunId: nativeRun.id,
      projectId: nativeRun.projectId,
    }
    const changeSet = {
      id: 'coding-change-set-1',
      stateVersion: 2 as const,
      codingRunId: nativeRun.id,
      projectId: nativeRun.projectId,
      workspaceId: nativeWorkspace.id,
      phase: 'initial' as const,
      executorVersion: 2 as const,
      configVersion: configuration.version,
      providerId: configuration.providerId,
      createdAt: '2026-08-29T09:01:00.000Z',
      expiresAt: '2026-08-29T09:16:00.000Z',
      changes: [{
        path: 'src/export.ts',
        expectedFileDigest: 'a'.repeat(64),
        replacements: [{ oldText: 'false', newText: 'true' }],
      }],
      unifiedDiff: 'diff --git a/src/export.ts b/src/export.ts\n',
    ...
    const second = await createLocalStore({ dbPath })
    await expect(second.getCodingRuntimeConfiguration(project.id)).resolves.toEqual(configuration)
    await second.saveCodingAgentRun(nativeRun)
    await second.saveManagedCodingWorkspace(nativeWorkspace)
    await expect(second.saveCodingChangeSet(changeSet)).resolves.toEqual(changeSet)
    await expect(second.saveCodingChangeSet({
      ...changeSet,
      unifiedDiff: 'tampered display diff',
    })).rejects.toThrow('Coding Change Set is immutable')
    await expect(second.saveCodingRuntimeConfiguration({
      ...configuration,
      providerId: 'another-provider',
      version: 2,
      updatedAt: '2026-08-29T09:02:00.000Z',
    })).rejects.toThrow('locked by active run')
    second.close()

    const reopened = await createLocalStore({ dbPath })
    await expect(reopened.listCodingRuntimeConfigurations()).resolves.toEqual([configuration])
    await expect(reopened.getCodingChangeSet(changeSet.id)).resolves.toEqual(changeSet)
    reopened.close()
  })

  it('persists only an exact confirmed OpenCode project configuration', async () => {
    const dbPath = await tempDbPath()
    const store = await createLocalStore({ dbPath })
    await store.upsertProject(project)
    const configuration = {
      ...
      binaryPath: path.resolve('/opt/devflow/bin/opencode'),
      detectedVersion: '1.2.3',
      version: 1,
      updatedAt: '2026-08-30T18:00:00.000Z',
    }

    await expect(store.saveCodingRuntimeConfiguration(configuration)).resolves.toEqual(configuration)
    await expect(store.getCodingRuntimeConfiguration(project.id)).resolves.toEqual(configuration)
    await expect(store.saveCodingRuntimeConfiguration({
      ...configuration,
      binaryPath: 'opencode',
      version: 2,
      updatedAt: '2026-08-30T18:01:00.000Z',
    })).rejects.toThrow('Coding Runtime configuration is invalid')
    store.close()
  })
  ...
  it('permanently redacts legacy Coding Agent event messages and metadata', async () => {
    ...
    const legacyDb = new SQL.Database(await readFile(dbPath))
    legacyDb.run('update coding_agent_events set json = ?, privacy_version = null where id = ?', [
      JSON.stringify(legacyEvent),
      legacyEvent.id,
    ])
    await writeFile(dbPath, Buffer.from(legacyDb.export()))
