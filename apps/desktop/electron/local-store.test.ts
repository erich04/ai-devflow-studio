...
describe('createLocalStore', () => {
  it('initializes schema version 16 and keeps it stable across reopen', async () => {
    const dbPath = await tempDbPath()

    const first = await createLocalStore({ dbPath })
    expect(await first.getSchemaVersion()).toBe(16)
    first.close()

    const second = await createLocalStore({ dbPath })
    expect(await second.getSchemaVersion()).toBe(16)
    second.close()
  })
  ...
  it('migrates an existing v1 database to v16 without losing local projects or runs', async () => {
    const dbPath = await tempDbPath()
    ...
    const db = new SQL.Database(await readFile(dbPath))
    expect(db.exec("select value from schema_meta where key = 'schema_version'")[0]?.values[0]?.[0]).toBe('16')
  ...
  it('migrates v8 data into a metadata-only remote-sync outbox schema', async () => {
    ...
    expect(await migrated.getSchemaVersion()).toBe(16)
  ...
  it('refuses to open a database created by a newer schema version', async () => {
    ...
    newerDb.run("update schema_meta set value = '17' where key = 'schema_version'")
    await writeFile(dbPath, Buffer.from(newerDb.export()))
    newerDb.close()

    await expect(createLocalStore({ dbPath })).rejects.toThrow(
      /schema version 17 is newer than supported version 16/,
    )
