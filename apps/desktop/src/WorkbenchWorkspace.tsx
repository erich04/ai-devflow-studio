...
import { ArrowUp, BookOpen, History, MessageCircle, Pin, Plus, RotateCcw, Square, X } from 'lucide-react'
...
function ConversationView({ session, runs, projectName, providerId, providerName, command, onNavigate, onConfigure }: {
  ...
  const [memory, setMemory] = useState(session.memory)
  ...
  const unreportedCalls = session.messages.filter((message) => message.role === 'notice' && message.provider && !message.usage).length
  ...
  return <>
    <header className="conversation-head">
      ...
      <button className="conversation-context-toggle" onClick={() => setContextOpen(!contextOpen)} aria-expanded={contextOpen}><BookOpen size={15} />上下文与会话记忆<span>{contextOpen ? '收起' : '查看'}</span></button>
      {contextOpen && <div className="conversation-context">
        ...
        <p>共享：最新流程、已保存产物与证据、项目代码和知识。<br />独立：此会话的消息、问题、草稿和以下记忆。</p>
        <label>仅本会话记忆<textarea aria-label="仅本会话记忆" value={memory} maxLength={6000} placeholder="可保留这条会话的目标、约束和已确认结论…" onChange={(event) => setMemory(event.target.value)} onBlur={() => { if (memory !== session.memory) void command({ type: 'update', ...scope, memory }) }} /></label>
        ...
        {session.contextReceipt?.limited && <p className="meta">本次上下文达到容量限制，部分记忆或查询内容未全部附带。可以缩小问题范围后继续调查。</p>}
        <p className="meta">当前工具：流程查询、节点与产物、只读代码检索、项目知识检索。没有连接业务数据库。</p>
      </div>}
    </header>
    <div className="conversation-messages" aria-label="当前会话消息" aria-busy={busy}>
      {session.messages.filter((message) => message.role !== 'notice').length === 0 && <div className="conversation-welcome">
...
function ConversationMessageView({ message, busy, targetLabel, onAnswer, onNavigate, onPublish }: {
  ...
  return <article className={`conversation-message conversation-message--${message.role}`}>
    ...
    {message.question && <div className="conversation-question"><span className="meta">{message.question.answeredAt ? '已收到后续回复' : '需要你补充'}</span><strong>{message.question.prompt}</strong>
