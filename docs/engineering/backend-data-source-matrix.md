...
| Agent Memory lifecycle | `listAgentMemoryLifecycle` | `real IPC/API` + `local persisted` | renderer 仅发送 selected Run、persisted Runtime 与 Local Project ID；Electron main 从 exact Runtime 派生完整 user/session scope，重验 canonical Run，并在读取 revision/tombstone 前后重验 current pairing，再投影有界 Candidate pending/promoted、Durable active/conflict/expired/purge/deleted 与 exact revision/head version。scope session、opaque capability、authority digest、raw output、local path 与 deleted statement 不出 main。 |
