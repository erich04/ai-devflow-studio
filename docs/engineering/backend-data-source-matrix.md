...
当前持久化基线是 Team schema v15 与 Desktop schema v18。Team 持久层记录非秘密的
...
| Agent Runtime | `startAgentRuntime` / `advanceAgentRuntime` / `cancelAgentRuntime` / `listAgentRuntimes` | `real IPC` + `local persisted` | Desktop schema v18 已持久化严格 trajectory、checkpoint、evaluation 与 terminal summary；当前仅 deterministic no-side-effect fake action，Native Tool/MCP 留给后续切片。 |
