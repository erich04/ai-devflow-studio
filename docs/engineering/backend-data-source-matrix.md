...
| 数据 | 当前来源 | 边界 |
| --- | --- | --- |
| 会话、输入、问题、记忆、Tab 开闭 | Electron `workbenchConversation` → SQLite `workbench_conversations`（schema 35） | 按 Local Project 隔离；不进入 `LocalExecutionState` 或 Team 同步。 |
| 全项目流程与任意节点 | Main 读取 canonical Run、Artifact、Test Evidence、Event、Coding/Delivery 和 Gate evaluation | 每次提问重新查询；不受选中卡片约束；完成状态不从聊天推断。 |
| 代码与知识依据 | 受限相对路径读取/搜索及现有 Repository Knowledge resolver | 只读；来源、时间、截断可见；未连接任意业务数据库。 |
