...
更新时间：2026-06-19  
适用版本：`v0.8.1` release candidate。当前代码已完成 v0.8 Policy-Aware Delivery 与 PR #2 smoke hardening；package metadata 仍为 `0.7.5`，将在 v0.8.1 walkthrough 通过后统一 bump 到 `0.8.1` 并打 tag。
...
本轮已通过的自动验证（2026-06-19）：

```bash
corepack pnpm verify
corepack pnpm build
DEVFLOW_DATABASE_URL=postgresql://erich@127.0.0.1:55435/devflow_v081_smoke corepack pnpm test:postgres-smoke
```

其中：

- `verify` 包含 typecheck、215 个 unit tests、cross-platform checks、3 个 browser E2E、Electron smoke。
