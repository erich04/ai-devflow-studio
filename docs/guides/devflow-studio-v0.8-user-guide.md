...
更新时间：2026-06-18  
适用版本：`0.7.5` package metadata，产品里程碑已完成到 `v0.8 Policy-Aware Delivery`

## 结论

DevFlow Studio 现在已经具备从 `v0.1` 到 `v0.8` 的主流程能力：团队开发 workflow、真实 Electron 本地执行、测试证据、知识治理、Knowledge Review Agent、Coding Agent adapter、Gate Enforcement、以及 v0.8 的 remediation/retry 闭环。
...
- 自动化签收：`verify`、`build`、Electron smoke、Postgres smoke 已在 v0.8 提交后通过。
...
corepack pnpm build
DEVFLOW_DATABASE_URL=postgres://<temp-user>@127.0.0.1:<temp-port>/devflow_v08 corepack pnpm test:postgres-smoke
```

其中：

- `verify` 包含 typecheck、unit tests、cross-platform checks、browser E2E、Electron smoke。
- `build` 覆盖 API、worker、desktop renderer/electron/preload、web。
- `postgres-smoke` 使用一次性本地 Postgres，覆盖迁移、seed、policy/evaluate/override/stale-version、overview audit。
