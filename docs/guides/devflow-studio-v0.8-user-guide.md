...
- 自动化签收：`verify`、`build`、Electron smoke、Postgres smoke 已在 v0.8.1 release-candidate 分支 `aa468cb` 上通过。
...
注意：当前 Computer Use 插件在本机能读屏，但 click 调用会返回 `noWindowsAvailable`（早期也出现过 “Computer Use is not active”）。所以本轮“点击模拟”由 Playwright 执行，真实 Electron 路径由 `test:electron-smoke` 覆盖。这个限制属于当前 Computer Use 工具层，不是 DevFlow UI 功能缺失。
...
- `verify` 包含 typecheck、238 个 unit tests、cross-platform checks、3 个 browser E2E、Electron smoke。
...
| 2 | 工作台 | 选中当前 Run，再点击 `方案评审 Gate` | Inspector 显示 Gate Enforcement、policy source、blocking reason |
...
2. 在工作台选中 `方案评审 Gate`，展示 Gate Enforcement 和 Remediation Plan。
