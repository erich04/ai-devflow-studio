...
| 多 Agent 协调 | 当前 Run / 节点的固定 `bounded-repair-v1` 验收图；首次只创建监督 Agent、任务图和检查点 | 首次创建不调用阶段模型服务商；后续手动启动的专职 Agent 可能在上限内消耗 token / 费用 | 分析角色只读；后续有界实现 Agent 可能只写受管工作区 | 不生成当前阶段产物，不推进工作流，不审批门禁 | 本地项目必须已配对团队项目；未配对时入口禁用并显示原因 |
