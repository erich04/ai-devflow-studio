...
describe('V1.5 Docker lifecycle smoke contract', () => {
  ...
  it('proves fresh schema v12 and a volume-retained V1.4 schema v10 upgrade', () => {
    ...
    expect(lifecycleSmoke).toContain("expectSchemaVersion(FRESH_DATABASE, 12)")
    ...
    expect(lifecycleSmoke).toContain("expectSchemaVersion(UPGRADE_DATABASE, 12)")
    expect(lifecycleSmoke).toContain('V1.4 retained sentinel')
    expect(lifecycleSmoke).toContain('snapshotBeforeV10Upgrade')
    expect(lifecycleSmoke).toContain('snapshotAfterV12Upgrade')
  ...
  it('fails the exact V1.4 API closed on v12, then proves authenticated backup restore on v10', () => {
