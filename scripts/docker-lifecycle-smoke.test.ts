...
describe('V1.5 Docker lifecycle smoke contract', () => {
  ...
  it('proves the fresh current schema and a volume-retained V1.4 schema v10 upgrade', () => {
    ...
    expect(lifecycleSmoke).toContain('const currentSchemaVersion = Number.parseInt(')
    expect(lifecycleSmoke).toContain('TEAM_SCHEMA_VERSION')
    expect(lifecycleSmoke).toContain('expectSchemaVersion(FRESH_DATABASE, currentSchemaVersion)')
    ...
    expect(lifecycleSmoke).toContain('expectSchemaVersion(UPGRADE_DATABASE, currentSchemaVersion)')
  ...
  it('migrates a legacy issued v12 credential through the current schema without inventing Runtime, Memory, or Coordination proof', () => {
    ...
    expect(lifecycleSmoke).toContain('expectSchemaVersion(FAILURE_DATABASE, currentSchemaVersion)')
    ...
    expect(lifecycleSmoke).toContain('assertAgentMemoryProjectionQualityAfterV18')
    expect(lifecycleSmoke).toContain('memory_id,head_version,quality_version')
    expect(lifecycleSmoke).toContain(
      '0018_agent_memory_projection_quality_version',
    )
    expect(lifecycleSmoke).toContain('assertAgentCoordinationProjectionAfterV19')
    expect(lifecycleSmoke).toContain('agent_coordination_summaries')
    expect(lifecycleSmoke).toContain('agent_coordination_projection_audits')
    expect(lifecycleSmoke).toContain(
      '0019_agent_coordination_team_projection',
    )
    expect(lifecycleSmoke).toContain(
    ...
    expect(lifecycleSmoke).toContain(
      'fresh current schema v${currentSchemaVersion}, retained V1.4 schema v10 upgrade',
    )
    expect(lifecycleSmoke).toContain(
  ...
  it('documents the bounded API and Desktop rollback windows', () => {
    ...
    expect(guide).toContain('V1.5 生命周期与回滚矩阵')
    expect(guide).toContain('V1.4 API')
    expect(guide).toContain('团队模式 v15')
    expect(guide).toContain('拒绝就绪')
    expect(guide).toContain('不得对其运行 V1.4 迁移器')
    expect(guide).toContain('桌面模式 v17')
    expect(guide).toContain('V1.4 模式 v12')
    expect(guide).toContain('恢复升级前备份')
  })
})
