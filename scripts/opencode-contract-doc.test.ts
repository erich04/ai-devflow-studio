...
describe('opencode runtime contract refresh documentation', () => {
  ...
  it('documents the release-only real provider smoke gate', () => {
    ...
    expect(releaseGate).toContain('exactly three credited provider segments')
    expect(releaseGate).toContain('bash-only, edit-only, and completion-only')
    expect(releaseGate.replace(/\s+/g, ' ')).toContain('zero blocked')
    expect(releaseGate).toContain('exactly two real permission requests')
    ...
    expect(v14ReleasePlan).toContain('exactly three credited provider segments')
    expect(v14ReleasePlan).toContain('bash-only, edit-only, and completion-only')
