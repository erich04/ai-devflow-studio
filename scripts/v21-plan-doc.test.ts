...
describe('V2.1 Evaluated Retrieval and Memory contract', () => {
  ...
  it('advances the single active Roadmap priority to V2.1 Slice 4 after durable indexing passes', () => {
    const roadmap = read('docs/roadmap.md')
    const plan = read('docs/plans/v2.1-evaluated-retrieval-memory.md')
    const testing = read('docs/engineering/testing-strategy.md')

    expect(plan).toContain('Status: Active — Slice 3 complete; Slice 4 next')
    expect(plan).toContain('| Slice 1 | Complete |')
    expect(plan).toContain('| Slice 2 | Complete |')
    expect(plan).toContain('| Slice 3 | Complete |')
    expect(plan).toContain('| Slice 4 | Next |')
    expect(plan).toContain('retained Desktop schema 21 → 22 migration with zero fabricated index rows')
    expect(plan).toMatch(/143 focused local-store\s+tests and 43 focused shared retrieval tests/u)
    expect(plan).toMatch(/192 test files and 2729\s+tests/u)
    expect(roadmap).toContain('### Now — Implement V2.1 Scoped Agent Memory Lifecycle')
    expect(roadmap).toContain('| Active milestone | V2.1 Slice 4 — scoped Agent Memory lifecycle |')
    expect(roadmap).toContain('| Next gate | Prove candidate, promotion, revision, conflict, expiry, deletion, and replay-safe purge semantics |')
