...
describe('V2.1 Evaluated Retrieval and Memory contract', () => {
  ...
  it('keeps the single active Roadmap priority on V2.1 Slice 6 after packaged Context completion', () => {
    const roadmap = read('docs/roadmap.md')
    const plan = read('docs/plans/v2.1-evaluated-retrieval-memory.md')
    ...
    expect(plan).toContain('| Slice 6 | In progress |')
    ...
    expect(roadmap).toContain('### Now — Add V2.1 Redacted Team Projection')
    expect(roadmap).toContain('| Active milestone | V2.1 Slice 6 — Redacted Team Projection |')
    expect(roadmap).toContain('| Next gate | Add retained Team schema 17 and exact metadata-only Memory projection |')
